...
metrics = ['Micro F1', 'Macro F1', 'Micro\nRecall', 'Micro\nPrecision']
...
axes[0].set_title('Performance Metrics on OOD Test Set', fontsize=20, weight='bold', pad=15)
axes[0].set_xticks(x)
axes[0].set_xticklabels(metrics, fontsize=18)
# Adjusted ylim slightly to accommodate the new highest value (approx 0.58) nicely
axes[0].set_ylim(0.2, 0.7) 
axes[0].legend(loc='upper left', frameon=True, framealpha=0.9, fontsize=18)
...
axes[1].set_ylabel('Match Rate (%)', fontsize=20)
axes[1].set_title('Exact Match Rate', fontsize=20, weight='bold', pad=15)
axes[1].set_xticks(x_em)
axes[1].set_xticklabels(em_labels, fontsize=18)
...
# Save the figure
filename = "experiment_ood_conditional_generation_metrics_updated_large.png"
