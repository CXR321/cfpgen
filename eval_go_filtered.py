...
@ck.option(
    '--exclude-file', '-ef', default='/AIRvePFS/dair/chenxr-data/repo/dplm/overlapping_uniprot_ids.txt',
    help='File containing Uniprot IDs to exclude')
def main(data_root, ont, test_predictions, exclude_file):
    ...
    common_uids = [uid for uid in preds.keys() if uid in gts and uid not in excluded_ids]
    ...
    print(f"after filtering and exclusion, len(common_uids), len(gt_list), len(pred_list): {len(common_uids)}, {len(gt_list)}, {len(pred_list)}")
    ...
    output_log = test_predictions.replace('.tsv', '_filtered_go-eval.log')
