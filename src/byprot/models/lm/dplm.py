...
class DiffusionProteinLanguageModel(nn.Module):
    ...
    def generate_step(self, batch, tokenizer=None,
                 max_iter=None, temperature=None,
                 partial_masks=None,
                 step=None,
                 sampling_strategy='gumbel_argmax',
                 compute_gradient_fn=None,
                 update_sequence_with_gradient=None,
                 eta=0,
                 device=None,
                 go_model=None, go_id=None, terms_dict=None):
        ...
        gradient = compute_gradient_fn(output_tokens, go_model, go_id, terms_dict, device)
        updated_tokens, updated_scores = update_sequence_with_gradient(output_tokens, output_scores, gradient, eta)
        ...
        output_masks, result_tokens, result_scores = self._reparam_decoding(
            output_tokens=prev_decoder_out['output_tokens'].clone(),
            output_scores=prev_decoder_out['output_scores'].clone(),
            cur_tokens=updated_tokens.clone(),
            cur_scores=updated_scores.clone(),  # todo: 实际只更新了score
            decoding_strategy='reparam-uncond-deterministic-linear',#'reparam-uncond-stochastic1.0-linear'
            xt_neq_x0=prev_decoder_out['output_masks'],
            non_special_sym_mask=non_special_sym_mask,
            t=step + 1,
        ...
        prev_decoder_out.update(
            ...
            step=step + 1,
            history=decoder_out.get('history', []),
        )
