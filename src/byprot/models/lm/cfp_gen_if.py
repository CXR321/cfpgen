...
from byprot.models.lm.modules.cfpgen_adapter import DPLMWithConditionalAdatper, DPLMWithAdapterConfig
import numpy as np
...
def load_state_dict_with_report(net, state_dict, strict=False):
    ...
    print(f"GVPTransformer successfully loaded {loaded_keys}/{total_keys_in_model} parameters.")
...
class ConditionalDPLM(nn.Module):
    ...
    def __init__(self, cfg) -> None:
        ...
        if 'cath' in cfg.decoder['pretrained_model_name_or_path']: # load pretrained structure encoder
            state_dict = torch.load(cfg.decoder['pretrained_model_name_or_path'], map_location='cpu')['state_dict']
            loaded_keys, missing_keys, unexpected_keys = load_state_dict_with_report(self.encoder, state_dict, strict=False)
            train_pnames = []
            for pname, param in self.encoder.named_parameters():
                if pname in ['out_proj.weight', 'out_proj.bias']:
    ...
    def forward_decoder(self, prev_decoder_out, encoder_out, need_attn_weights=False, partial_masks=None,
        ...
        if 'go_type' in prev_decoder_out:
            batch.update({
                'go_type': prev_decoder_out['go_type'],
            })

        if 'ipr_type' in prev_decoder_out:
            batch.update({
                'ipr_type': prev_decoder_out['ipr_type']
            })

        if 'ec_type' in prev_decoder_out:
            batch.update({
                'ec_type': prev_decoder_out['ec_type']
            })
    ...
    def generate(self, batch, tokenizer=None,
        ...
        # todo debug: handle multiple seqs at once
        ...
        if 'go_type' in batch:
            prev_decoder_out['go_type'] = batch['go_type']
        if 'ipr_type' in batch:
            prev_decoder_out['ipr_type'] = batch['ipr_type']
        if 'ec_type' in batch:
            prev_decoder_out['ec_type'] = batch['ec_type']
