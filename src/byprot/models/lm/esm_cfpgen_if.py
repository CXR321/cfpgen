...
class AGFMLayer(nn.Module):
    def __init__(self, config):
        ...
        self.adaLN_modulation = nn.Sequential(
            nn.SiLU(),
            # nn.Linear(config.hidden_size, 6 * config.hidden_size, bias=True),  # if use gate_msa
            nn.Linear(config.hidden_size, 4 * config.hidden_size, bias=True),  # zero-shot runs better w/o gate_msa
        )
    ...
    def forward(
        ...
        if cond_input is not None:
            if not cond_input.shape[0] == hidden_states.shape[0]:  # for different T during training
                cond_input = cond_input.repeat(2, 1)
...
class AGFMAdapter(nn.Module):
    def __init__(self, config, func_tag=False):
        super().__init__()
        self.seq_len_dim = 1
        self.attention = AGFMAttention(config)
        self.intermediate = EsmIntermediate(config)
    ...
    def forward(
        ...
        if cond_input is not None and self.use_modulation:
            if not cond_input.shape[0] == hidden_states.shape[0]:
                cond_input = cond_input.repeat(2, 1)
            # shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(cond_input).chunk(6, dim=1)
            shift_msa, scale_msa, shift_mlp, scale_mlp = self.adaLN_modulation(cond_input).chunk(4, dim=1)
            gate_msa = gate_mlp = None
        ...
        # if (not self.training_flag) or random.random() > self.struc_cond_dropout_prob or (cond_input.shape[0] == hidden_states.shape[0]):
        # if (not self.training_flag) or random.random() > self.struc_cond_dropout_prob:
        ...
        cross_attention_output = cross_attention_outputs[0]  # tensor(-633848.0625, device='cuda:0')
...
class ModifiedEsmModel(EsmModel):
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            try:
                input_shape = input_ids['prev_tokens'].size()
        ...
        if not torch.is_tensor(input_ids):
            device = input_ids['prev_tokens'].device if input_ids is not None else inputs_embeds.device
        else:
        ...
        embedding_output = self.embeddings(  # tensor(-46189.2344, device='cuda:0')
            input_ids=input_ids if torch.is_tensor(input_ids) else input_ids['prev_tokens'],  # tensor(35927, device='cuda:0')
        ...
        encoder_outputs = self.encoder(
            ...
            anno_tag=input_ids if isinstance(input_ids, dict) else None,
        )
...
class EsmForCFPGenInverseFolding(EsmForMaskedLM):
    ...
    def forward(self,
        ...
        if not torch.is_tensor(input_ids):
            attention_mask = input_ids['prev_tokens'].ne(self.pad_id)
        else:
