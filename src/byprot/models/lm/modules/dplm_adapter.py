    ...
    def from_pretrained(cls, cfg):
        ...
        for pname, param in dplm_adapter.named_parameters():
            if 'adapter' not in pname:
                param.requires_grad = False  # todo debug
            else:
...
class AdapterLayer(nn.Module):
    def __init__(self, cfg, config):
        ...
        self.training_flag = False  # todo debug: False in eval model
        self.struc_cond_dropout_prob = getattr(cfg, 'adapter_dropout', 0.0)
    ...
    def forward(
        ...
        if (not self.training_flag) or random.random() > self.struc_cond_dropout_prob:
            ...
            cross_attention_output = cross_attention_outputs[0]  # tensor(-635332.2500, device='cuda:0')
        ...
        return outputs  # tensor(-1279466.6250, device='cuda:0')
