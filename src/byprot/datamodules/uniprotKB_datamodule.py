...
# @register_datamodule('uniprotKB')
# class UniprotKBDataModule(LightningDataModule):
#     def __init__(
#         self,
...
#         self.hparams.mini_run = self.hparams.data_dir.mini_run
...
#         if stage == 'fit':
#             self.train_dataset = UniProtKBDataset(data_dir=self.hparams.data_dir, split='train', max_len=self.hparams.max_len)
#             self.valid_dataset = UniProtKBDataset(data_dir=self.hparams.data_dir, split='valid', max_len=self.hparams.max_len)
...
#             self.train_dataset,
#             # collater=self.hparams.collater,
#             max_tokens=self.hparams.data_dir.max_tokens,
#             num_workers=self.hparams.data_dir.num_workers,
#             mini_run=self.hparams.data_dir.mini_run,
#             max_len=self.hparams.data_dir.max_len,
#             max_batch_size=1 if self.stage == 'test' or self.stage == 'predict' else 800,
#         )
...
@register_datamodule('uniprotKB_dplm2')
class UniprotKBDataModuleDPLM2(LightningDataModule):
    def __init__(
        self,
        data_dir: str = "data/tape",
        max_tokens: int = 6000,
        ...
        self.use_go_null_token = self.hparams.data_dir.use_go_null_token
        print(f"self.use_go_null_token {self.use_go_null_token}")
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        # load datasets only if they're not loaded already
        if stage == 'fit':
            self.train_dataset = UniProtKB_DPLM2_Dataset(data_dir=self.hparams.data_dir, split='train', max_len=self.hparams.max_len)
            self.valid_dataset = UniProtKB_DPLM2_Dataset(data_dir=self.hparams.data_dir, split='valid', max_len=self.hparams.max_len)
            if self.hparams.mini_run:
                mini_size = 1
                # subsample trainset
                tindices = np.arange(0,1000)
    ...
    def train_dataloader(self):
        ...
        return setup_dataloader_dplm2(
            ...
            max_len=self.hparams.data_dir.max_len,
            max_batch_size=1 if self.stage == 'test' or self.stage == 'predict' else 800,
            use_go_null_token=self.use_go_null_token,
        )
